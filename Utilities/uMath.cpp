...
Matrix & Matrix::SetPerspectiveInverse(GLfloat fovY, GLfloat aspect, GLfloat nearPlane, GLfloat farPlane)
{
	GLfloat height = 2.0f * nearPlane * tanf(fovY * 0.5f);
	GLfloat width = height * aspect;
	GLfloat n2 = 2.0f * nearPlane;
	...
	float C = (farPlane + nearPlane) * rcpnmf;
	float D = farPlane * rcpnmf * n2;
	m[0][2] = 0;
	m[1][2] = 0;
	m[2][2] = 0;
	m[3][2] = -1;
